...
interface LiveWorldMapProps {
  ...
  onSelectCivilAlert?: (alert: any) => void;
  onSelectWaterwayGauge?: (gauge: any) => void;
}
...
function LiveWorldMap({ data, activeLayers, onEntityClick, onMouseCoords, onRightClick, onViewStateChange, flyToLocation, projection = 'globe', terrainEnabled = false, terrainRetry = 0, terrainFocus = 0, onTerrainStatusChange, mapStyle = 'dark', sweepData, scanTargets = [], demoMode = false, theme = 'core', drawnPolygons = [], arcgisLayers = [], drawMode = null, onDrawComplete, onDrawProgress, onDrawCancel, drawCommand = null, onMapCenter, route = null, userLocation = null, followUser = false, onFollowInterrupt, navigating = false, aircraftAirports = {}, onSelectStation, onSelectCivilAlert, onSelectWaterwayGauge }: LiveWorldMapProps) {
  ...
  const createIcon = useCallback((map: maplibregl.Map, id: string, color: string, size: number) => {
    ...
    const icon = renderAircraftImageData(id, color, Math.max(size, 28));
  ...
  useEffect(() => {
    ...
    map.on('load', () => {
      ...
      map.addLayer({ id: 'grid-flow-lines', type: 'line', source: 'grid-flows', paint: {
        'line-color': ['case', ['>=', ['get','mw'], 2500], '#FFB300', '#00E5FF'],
        'line-width': ['interpolate',['linear'],['get','mw'], 0,1.2, 1000,2.2, 5000,5],
        'line-opacity': 0.78, 'line-blur': 0.25,
      }});
      map.addLayer({ id: 'grid-flow-label', type: 'symbol', source: 'grid-flows', minzoom: 3, layout: {
        'symbol-placement': 'line-center', 'text-field': ['concat',['to-string',['get','mw']],' MW'],
        'text-size': 9, 'text-font': ['JetBrains Mono Bold', 'Open Sans Bold'],
      }, paint: { 'text-color': '#7FE9FF', 'text-halo-color': '#000', 'text-halo-width': 1.5 }});
      map.addLayer({ id: 'grid-frequency-halo', type: 'circle', source: 'grid-frequency', paint: {
        'circle-radius': ['interpolate',['linear'],['zoom'], 1,14, 6,25],
        'circle-color': ['match',['get','status'],'critical','#FF1744','strained','#FFB300','#00E676'],
        'circle-opacity': 0.12, 'circle-blur': 0.9,
      }});
      map.addLayer({ id: 'grid-frequency-dot', type: 'circle', source: 'grid-frequency', paint: {
        'circle-radius': ['interpolate',['linear'],['zoom'], 1,5, 6,8],
        'circle-color': ['match',['get','status'],'critical','#FF1744','strained','#FFB300','#00E676'],
        'circle-opacity': 0.95, 'circle-stroke-width': 2, 'circle-stroke-color': '#000',
      }});
      map.addLayer({ id: 'grid-frequency-label', type: 'symbol', source: 'grid-frequency', minzoom: 3, layout: {
        'text-field': ['concat',['to-string',['get','hz']],' Hz'], 'text-size': 10,
        'text-font': ['JetBrains Mono Bold', 'Open Sans Bold'], 'text-offset': [0,1.6],
      }, paint: { 'text-color': '#E8E6E0', 'text-halo-color': '#000', 'text-halo-width': 1.5 }});
      map.addLayer({ id: 'grid-outage-dots', type: 'circle', source: 'grid-outages', paint: {
        'circle-radius': ['interpolate',['linear'],['get','unavailable_mw'], 0,4, 500,7, 2000,13],
        'circle-color': ['match',['get','fuel'],'Nuclear','#FF1744','#FF9500'], 'circle-opacity': 0.8,
        'circle-stroke-width': 1.5, 'circle-stroke-color': '#FFD0C8', 'circle-stroke-opacity': 0.65,
      }});

      // Signal & airspace intelligence. GNSS cells are statistical ADS-B
      // integrity anomalies, not a claim that a jammer has been located.
      map.addLayer({ id: 'gnss-integrity-halo', type: 'circle', source: 'gnss-interference', paint: {
        'circle-radius': ['interpolate',['linear'],['zoom'], 1,13, 6,30],
        'circle-color': ['match',['get','level'],'high','#FF1744','#FFB300'],
        'circle-opacity': 0.12, 'circle-blur': 0.75,
      }});
      map.addLayer({ id: 'gnss-integrity-dots', type: 'circle', source: 'gnss-interference', paint: {
        'circle-radius': ['interpolate',['linear'],['get','samples'], 3,4, 20,8, 100,13],
        'circle-color': ['match',['get','level'],'high','#FF1744','#FFB300'],
        'circle-opacity': 0.88, 'circle-stroke-width': 1.5, 'circle-stroke-color': '#FFE7A0',
      }});
      map.addLayer({ id: 'gnss-integrity-label', type: 'symbol', source: 'gnss-interference', minzoom: 4, layout: {
        'text-field': ['concat',['to-string',['get','affected_percent']],'% GNSS'], 'text-size': 9,
        'text-font': ['JetBrains Mono Bold','Open Sans Bold'], 'text-offset': [0,1.7],
      }, paint: { 'text-color': '#FFD166', 'text-halo-color': '#000', 'text-halo-width': 1.4 }});

      map.addLayer({ id: 'sdr-receiver-halo', type: 'circle', source: 'sdr-receivers', paint: {
        'circle-radius': ['interpolate',['linear'],['zoom'], 1,4, 7,10], 'circle-color': '#00E5FF',
        'circle-opacity': 0.1, 'circle-blur': 0.75,
      }});
      map.addLayer({ id: 'sdr-receiver-dots', type: 'circle', source: 'sdr-receivers', paint: {
        'circle-radius': ['interpolate',['linear'],['zoom'], 1,1.8, 7,4.5, 12,7],
        'circle-color': ['match',['get','receiver_type'],'KiwiSDR','#00E5FF','WebSDR','#B388FF','#26A69A'],
        'circle-opacity': 0.85, 'circle-stroke-width': 1, 'circle-stroke-color': '#DDFBFF',
      }});
      map.addLayer({ id: 'sdr-receiver-label', type: 'symbol', source: 'sdr-receivers', minzoom: 7, layout: {
        'text-field': ['get','receiver_type'], 'text-size': 8, 'text-font': ['Open Sans Regular'],
        'text-offset': [0,1.5], 'text-allow-overlap': false,
      }, paint: { 'text-color': '#7FE9FF', 'text-halo-color': '#000', 'text-halo-width': 1.2 }});

      map.addLayer({ id: 'notam-alert-halo', type: 'circle', source: 'notam-alerts', paint: {
        'circle-radius': ['interpolate',['linear'],['zoom'], 1,10, 7,22],
        'circle-color': ['match',['get','severity'],'critical','#FF1744','high','#FF9500','#FFD500'],
        'circle-opacity': 0.1, 'circle-blur': 0.7,
      }});
      map.addLayer({ id: 'notam-alert-dots', type: 'circle', source: 'notam-alerts', paint: {
        'circle-radius': ['interpolate',['linear'],['zoom'], 1,4, 7,8],
        'circle-color': ['match',['get','severity'],'critical','#FF1744','high','#FF9500','#FFD500'],
        'circle-opacity': 0.92, 'circle-stroke-width': 2, 'circle-stroke-color': '#FFF1DA',
      }});
      map.addLayer({ id: 'notam-alert-label', type: 'symbol', source: 'notam-alerts', minzoom: 4, layout: {
        'text-field': ['concat','NOTAM ',['get','location']], 'text-size': 9, 'text-font': ['Open Sans Bold'],
        'text-offset': [0,1.8], 'text-allow-overlap': false,
      }, paint: { 'text-color': '#FFB36B', 'text-halo-color': '#000', 'text-halo-width': 1.4 }});

      // Global Energy Monitor gas infrastructure.
      map.addLayer({ id: 'gas-pipeline-lines', type: 'line', source: 'gas-pipelines', paint: {
        'line-color': ['match',['get','status'],'operating','#FFB300','construction','#FF5A36','proposed','#B388FF','#657078'],
        'line-width': ['interpolate',['linear'],['zoom'], 1,0.8, 5,2, 10,4], 'line-opacity': 0.72,
      }});
      map.addLayer({ id: 'lng-terminal-halo', type: 'circle', source: 'lng-terminals', paint: {
        'circle-radius': ['interpolate',['linear'],['zoom'], 1,8, 6,16], 'circle-color': '#B388FF',
        'circle-opacity': 0.1, 'circle-blur': 0.8,
      }});
      map.addLayer({ id: 'lng-terminal-dots', type: 'circle', source: 'lng-terminals', paint: {
        'circle-radius': ['interpolate',['linear'],['zoom'], 1,3, 6,6, 10,9], 'circle-color': '#B388FF',
        'circle-opacity': 0.9, 'circle-stroke-width': 1.5, 'circle-stroke-color': '#E8DFFF',
      }});
      map.addLayer({ id: 'lng-terminal-label', type: 'symbol', source: 'lng-terminals', minzoom: 5, layout: {
        'text-field': ['get','name'], 'text-size': 9, 'text-font': ['Open Sans Regular'],
        'text-offset': [0,1.6], 'text-max-width': 13, 'text-allow-overlap': false,
      }, paint: { 'text-color': '#CBB7FF', 'text-halo-color': '#000', 'text-halo-width': 1.3 }});
      ...
      map.addLayer({ id: 'nina-alerts-glow', type: 'circle', source: 'nina-alerts', paint: {
        'circle-radius': ['interpolate',['linear'],['zoom'], 3,8, 7,16, 12,28],
        'circle-color': [
          'match', ['get', 'severity'],
          'Extreme', '#FF1744',
          'Severe', '#FF9100',
      ...
      map.addLayer({ id: 'nina-alerts-dots', type: 'circle', source: 'nina-alerts', paint: {
        'circle-radius': ['interpolate',['linear'],['zoom'], 3,4.5, 7,7, 12,12],
        'circle-color': [
          'match', ['get', 'severity'],
          'Extreme', '#FF1744',
          'Severe', '#FF9100',
      ...
      map.addLayer({ id: 'nina-alerts-label', type: 'symbol', source: 'nina-alerts', minzoom: 5, layout: {
      ...
      // River Gauges / WSV Pegelonline — river monitoring stations
      map.addLayer({ id: 'waterway-gauges-glow', type: 'circle', source: 'waterway-gauges', paint: {
        'circle-radius': ['interpolate',['linear'],['zoom'], 4,6, 8,12, 13,20],
        'circle-color': [
          'match', ['get', 'stage'],
          'high_2', '#FF1744',
          'high_1', '#FF9100',
      ...
      map.addLayer({ id: 'waterway-gauges-dots', type: 'circle', source: 'waterway-gauges', paint: {
        'circle-radius': ['interpolate',['linear'],['zoom'], 4,3, 8,5, 13,8],
        'circle-color': [
          'match', ['get', 'stage'],
          'high_2', '#FF1744',
          'high_1', '#FF9100',
        ...
        'circle-stroke-color': '#003344',
        'circle-stroke-opacity': 0.9,
      }});
      map.addLayer({ id: 'waterway-gauges-label', type: 'symbol', source: 'waterway-gauges', minzoom: 6, layout: {
        'text-field': ['concat', ['get', 'name'], ' · ', ['to-string', ['get', 'value']], 'cm'],
        'text-size': 9,
    ...
    ['sdk-sea','sdk-sea-glow','sdk-air','sdk-air-glow','sdk-intel','sdk-intel-glow'].forEach(layer => {
      map.on('click', layer, e => {
        ...
        const srcUrl = p.url || SDK_SOURCE_URLS[p.source] || window.location.origin;
    ...
    ['conflict-icons','cctv-dots','eq-circles','fires-heat','gdelt-dots','weather-dots','infra-dots','grid-frequency-dot','grid-flow-lines','grid-outage-dots','gas-pipeline-lines','lng-terminal-dots','gnss-integrity-dots','sdr-receiver-dots','notam-alert-dots','maritime-dots','choke-dots','news-dots','balloon-dots','rad-dots','ship-dots','sweep-device-dots','scan-targets-dots','sdk-sea','sdk-sea-glow','sdk-sea-atmo','sdk-air','sdk-air-glow','sdk-air-atmo','sdk-intel','sdk-intel-glow','sdk-intel-atmo','malware-dots','cyber-heads','gdelt-events-dots','cf-outage-dots','cf-attack-dots'].forEach(layer => {
      map.on('mouseenter', layer, () => { map.getCanvas().style.cursor = 'pointer'; });
      map.on('mouseleave', layer, () => { map.getCanvas().style.cursor = ''; });
    });
    ...
    // ── Signal & Airspace Intelligence ──
    map.on('click', 'gnss-integrity-dots', e => {
      if (!e.features?.length) return;
      const p = e.features[0].properties as any;
      const coords = (e.features[0].geometry as any).coordinates;
      const accent = p.level === 'high' ? '#FF1744' : '#FFB300';
      popup(coords, `<div style="${pStyle}border:1px solid ${accent}66;min-width:250px;">
        <div style="color:${accent};font-size:10px;font-weight:700;letter-spacing:.12em;margin-bottom:7px;">GNSS INTEGRITY ANOMALY</div>
        <div style="color:#E8E6E0;font-size:20px;font-weight:700;margin-bottom:7px;">${htmlEsc(p.affected_percent)}%</div>
        <div style="display:grid;grid-template-columns:1fr 1fr;gap:6px;font-size:9px;">
          <div><span style="color:#5C5A54;">LOW NACp</span><br/><span style="color:${accent};">${htmlEsc(p.affected)} / ${htmlEsc(p.samples)}</span></div>
          <div><span style="color:#5C5A54;">CONFIDENCE</span><br/><span style="color:#E8E6E0;">${htmlEsc(String(p.confidence).toUpperCase())}</span></div>
          <div><span style="color:#5C5A54;">AVG NACp</span><br/><span style="color:#E8E6E0;">${htmlEsc(p.average_nac_p)}</span></div>
          <div><span style="color:#5C5A54;">CELL</span><br/><span style="color:#E8E6E0;">${htmlEsc(p.cell_degrees)}° × ${htmlEsc(p.cell_degrees)}°</span></div>
        </div>
        <div style="margin-top:8px;font-size:8px;line-height:1.5;color:#7D7970;">Low ADS-B navigation accuracy can have multiple causes. This is not proof of jamming or spoofing.</div>
        <a href="${urlSafe(p.source_url)}" target="_blank" rel="noopener noreferrer" style="${linkStyle}color:${accent};border:1px solid ${accent}66;background:${accent}1a;">COMPARE GPSJAM</a>
      </div>`);
    });

    map.on('click', 'sdr-receiver-dots', e => {
      if (!e.features?.length) return;
      const p = e.features[0].properties as any;
      const coords = (e.features[0].geometry as any).coordinates;
      const receiverUrl = urlSafe(p.url);
      const directoryUrl = urlSafe(p.directory_url);
      popup(coords, `<div style="${pStyle}border:1px solid rgba(0,229,255,.4);min-width:260px;">
        <div style="color:#00E5FF;font-size:10px;font-weight:700;letter-spacing:.12em;margin-bottom:6px;">PUBLIC SDR RECEIVER</div>
        <div style="color:#E8E6E0;font-size:12px;font-weight:700;line-height:1.4;margin-bottom:7px;">${htmlEsc(p.name)}</div>
        <div style="font-size:9px;color:#9B978E;line-height:1.7;">${htmlEsc(p.receiver_type)}${p.version ? ` · v${htmlEsc(p.version)}` : ''}<br/>${htmlEsc(p.coverage)}<br/>${htmlEsc(p.location)}</div>
        <div style="margin-top:7px;font-size:8px;color:#5C5A54;">Operator-published position; it may be approximate. Respect receiver rules and local radio law.</div>
        ${receiverUrl !== '#' ? `<a href="${receiverUrl}" target="_blank" rel="noopener noreferrer" style="${linkStyle}color:#00E5FF;border:1px solid rgba(0,229,255,.4);background:rgba(0,229,255,.1);">OPEN RECEIVER</a>` : ''}
        ${directoryUrl !== '#' ? `<a href="${directoryUrl}" target="_blank" rel="noopener noreferrer" style="${linkStyle}color:#9B978E;border:1px solid rgba(155,151,142,.3);">DIRECTORY</a>` : ''}
      </div>`);
    });

    map.on('click', 'notam-alert-dots', e => {
      if (!e.features?.length) return;
      const p = e.features[0].properties as any;
      const coords = (e.features[0].geometry as any).coordinates;
      const accent = p.severity === 'critical' ? '#FF1744' : p.severity === 'high' ? '#FF9500' : '#FFD500';
      popup(coords, `<div style="${pStyle}border:1px solid ${accent}66;min-width:280px;max-width:340px;">
        <div style="color:${accent};font-size:10px;font-weight:700;letter-spacing:.12em;margin-bottom:6px;">OPERATIONAL NOTAM · ${htmlEsc(p.location)}</div>
        <div style="color:#E8E6E0;font-size:10px;line-height:1.5;max-height:150px;overflow:auto;white-space:pre-wrap;">${htmlEsc(p.message)}</div>
        <div style="margin-top:8px;font-size:8px;color:#8A867E;line-height:1.6;">${p.starts_at ? `START ${htmlEsc(p.starts_at)}<br/>` : ''}${p.ends_at ? `END ${htmlEsc(p.ends_at)}<br/>` : ''}Position: ${p.position_basis === 'notam-coordinate' ? 'coordinate encoded in notice' : 'approximate FIR/aerodrome anchor'}</div>
        <div style="margin-top:7px;font-size:8px;color:#5C5A54;">Filtered situational awareness only — verify with an official flight briefing.</div>
        <a href="${urlSafe(p.source_url)}" target="_blank" rel="noopener noreferrer" style="${linkStyle}color:${accent};border:1px solid ${accent}66;background:${accent}1a;">ICAO SOURCE</a>
      </div>`);
    });

    // ── European Grid & Gas Infrastructure ──
    map.on('click', 'grid-frequency-dot', e => {
      if (!e.features?.length) return;
      const p = e.features[0].properties as any;
      const coords = (e.features[0].geometry as any).coordinates;
      const accent = p.status === 'critical' ? '#FF1744' : p.status === 'strained' ? '#FFB300' : '#00E676';
      popup(coords, `<div style="${pStyle}border:1px solid ${accent}66;min-width:240px;">
        <div style="color:${accent};font-size:14px;font-weight:700;margin-bottom:4px;">⚡ ${Number(p.hz).toFixed(3)} Hz</div>
        <div style="color:#9B978E;font-size:9px;margin-bottom:9px;">CONTINENTAL EUROPE · ${htmlEsc(String(p.status).toUpperCase())}</div>
        <div style="display:grid;grid-template-columns:1fr 1fr;gap:7px;font-size:9px;">
          <div><span style="color:#5C5A54;">DEVIATION</span><br/><span style="color:${accent};">${Number(p.deviation_mhz) > 0 ? '+' : ''}${htmlEsc(p.deviation_mhz)} mHz</span></div>
          <div><span style="color:#5C5A54;">MEASURED</span><br/><span style="color:#E8E6E0;">${htmlEsc(String(p.measured_at).slice(0,19).replace('T',' '))}Z</span></div>
        </div>
        <div style="margin-top:8px;font-size:8px;color:#5C5A54;">Fraunhofer ISE Energy-Charts · Freiburg measurement</div>
        <a href="${urlSafe(p.source_url)}" target="_blank" rel="noopener noreferrer" style="${linkStyle}color:${accent};border:1px solid ${accent}66;background:${accent}1a;">ENERGY-CHARTS</a>
      </div>`);
    });

    map.on('click', 'grid-flow-lines', e => {
      if (!e.features?.length) return;
      const p = e.features[0].properties as any;
      popup([e.lngLat.lng, e.lngLat.lat], `<div style="${pStyle}border:1px solid rgba(0,229,255,.35);min-width:230px;">
        <div style="color:#00E5FF;font-size:10px;font-weight:700;letter-spacing:.12em;margin-bottom:8px;">CROSS-BORDER PHYSICAL FLOW</div>
        <div style="color:#E8E6E0;font-size:13px;font-weight:700;">${htmlEsc(p.from)} → ${htmlEsc(p.to)}</div>
        <div style="color:#7FE9FF;font-size:20px;font-weight:700;margin:7px 0;">${Number(p.mw).toLocaleString()} MW</div>
        <div style="font-size:8px;color:#5C5A54;">ENTSO-E Transparency Platform · ${htmlEsc(String(p.measured_at).slice(0,16).replace('T',' '))}Z</div>
      </div>`);
    });

    map.on('click', 'grid-outage-dots', e => {
      if (!e.features?.length) return;
      const p = e.features[0].properties as any;
      const coords = (e.features[0].geometry as any).coordinates;
      popup(coords, `<div style="${pStyle}border:1px solid rgba(255,61,61,.4);min-width:240px;">
        <div style="color:#FF5A36;font-size:10px;font-weight:700;letter-spacing:.12em;margin-bottom:7px;">GENERATION UNAVAILABILITY</div>
        <div style="color:#E8E6E0;font-size:13px;font-weight:700;margin-bottom:8px;">${htmlEsc(p.name)}</div>
        <div style="display:grid;grid-template-columns:1fr 1fr;gap:7px;font-size:9px;">
          <div><span style="color:#5C5A54;">UNAVAILABLE</span><br/><span style="color:#FF5A36;font-weight:700;">${Number(p.unavailable_mw).toLocaleString()} MW</span></div>
          <div><span style="color:#5C5A54;">TYPE</span><br/><span style="color:#E8E6E0;">${htmlEsc(p.fuel)}</span></div>
          <div><span style="color:#5C5A54;">AREA</span><br/><span style="color:#E8E6E0;">${htmlEsc(p.country)}</span></div>
          <div><span style="color:#5C5A54;">ENDS</span><br/><span style="color:#E8E6E0;">${htmlEsc(String(p.ends_at).slice(0,16).replace('T',' '))}Z</span></div>
        </div>
        <div style="margin-top:8px;font-size:8px;color:#5C5A54;">ENTSO-E Transparency Platform</div>
      </div>`);
    });

    map.on('click', 'gas-pipeline-lines', e => {
      if (!e.features?.length) return;
      const p = e.features[0].properties as any;
      const ref = urlSafe(p.source_url);
      popup([e.lngLat.lng, e.lngLat.lat], `<div style="${pStyle}border:1px solid rgba(255,179,0,.4);min-width:250px;">
        <div style="color:#FFB300;font-size:10px;font-weight:700;letter-spacing:.12em;margin-bottom:6px;">GAS PIPELINE</div>
        <div style="color:#E8E6E0;font-size:13px;font-weight:700;margin-bottom:8px;">${htmlEsc(p.name)}</div>
        <div style="display:grid;grid-template-columns:1fr 1fr;gap:7px;font-size:9px;">
          <div><span style="color:#5C5A54;">STATUS</span><br/><span style="color:#FFB300;">${htmlEsc(String(p.status).toUpperCase())}</span></div>
          <div><span style="color:#5C5A54;">COUNTRIES</span><br/><span style="color:#E8E6E0;">${htmlEsc(p.countries || '—')}</span></div>
          <div><span style="color:#5C5A54;">OWNER</span><br/><span style="color:#E8E6E0;">${htmlEsc(p.owner || '—')}</span></div>
          <div><span style="color:#5C5A54;">CAPACITY</span><br/><span style="color:#E8E6E0;">${htmlEsc([p.capacity,p.capacity_units].filter(Boolean).join(' ') || '—')}</span></div>
        </div>
        <div style="margin-top:8px;font-size:8px;color:#5C5A54;">Global Energy Monitor · GGIT · CC BY 4.0</div>
        ${ref !== '#' ? `<a href="${ref}" target="_blank" rel="noopener noreferrer" style="${linkStyle}color:#FFB300;border:1px solid rgba(255,179,0,.4);background:rgba(255,179,0,.1);">SOURCE</a>` : ''}
      </div>`);
    });

    map.on('click', 'lng-terminal-dots', e => {
      if (!e.features?.length) return;
      const p = e.features[0].properties as any;
      const coords = (e.features[0].geometry as any).coordinates;
      const ref = urlSafe(p.source_url);
      popup(coords, `<div style="${pStyle}border:1px solid rgba(179,136,255,.45);min-width:230px;">
        <div style="color:#B388FF;font-size:10px;font-weight:700;letter-spacing:.12em;margin-bottom:6px;">LNG TERMINAL</div>
        <div style="color:#E8E6E0;font-size:13px;font-weight:700;margin-bottom:8px;">${htmlEsc(p.name)}</div>
        <div style="font-size:9px;color:#9B978E;line-height:1.7;">${htmlEsc(p.country || '—')} · ${htmlEsc(String(p.status).toUpperCase())}<br/>${htmlEsc([p.capacity,p.capacity_units].filter(Boolean).join(' ') || 'Capacity not reported')}</div>
        <div style="margin-top:8px;font-size:8px;color:#5C5A54;">Global Energy Monitor · GGIT · CC BY 4.0</div>
        ${ref !== '#' ? `<a href="${ref}" target="_blank" rel="noopener noreferrer" style="${linkStyle}color:#B388FF;border:1px solid rgba(179,136,255,.4);background:rgba(179,136,255,.1);">SOURCE</a>` : ''}
      </div>`);
    });
    ...
    map.on('click', 'station-dots', e => {
      ...
      if (onSelectStation) {
        onSelectStation({
          ...
          operator: p.operator,
          hasHighSpeed: p.hasHighSpeed,
        });
      }

      new maplibregl.Popup({ closeButton: true, maxWidth: '280px', className: 'osiris-popup' })
        .setLngLat([lng, lat])
        .setHTML(`
          <div style="font-family:monospace;font-size:11px;color:#fff;background:#0a0e17;padding:8px;border-radius:4px;border:1px solid #00E5FF">
            <div style="font-weight:bold;color:#00E5FF;margin-bottom:4px;display:flex;align-items:center;gap:4px">
              🚆 ${p.name}
            </div>
            <div style="color:#aaa;margin-bottom:4px">${p.city}, ${p.country}</div>
            <div style="display:flex;justify-content:space-between;margin-bottom:2px">
              <span style="color:#666">Gleise:</span>
              <span style="color:#fff;font-weight:bold">${p.tracks || '—'}</span>
            </div>
            <div style="display:flex;justify-content:space-between;margin-bottom:6px">
              <span style="color:#666">Passagiere:</span>
              <span style="color:#FFD700">${p.dailyPassengers || '—'} / Tag</span>
            </div>
            <div style="color:#26A69A;font-size:10px;text-align:center;border-top:1px solid #222;padding-top:4px">
              Live-Abfahrten im HUD geöffnet
            </div>
          </div>
        `)
        .addTo(map);
  ...
  useEffect(() => {
    if (!mapReady) return;
    setGeo('nina-alerts', (activeLayers as any).nina_alerts && data.nina_alerts ? data.nina_alerts.map((a: any) => ({
      type: 'Feature',
      geometry: { type: 'Point', coordinates: [a.lng, a.lat] },
      properties: {
        id: a.id,
    ...
    setGeo('waterway-gauges', (activeLayers as any).waterways && data.waterway_gauges ? data.waterway_gauges.map((g: any) => ({
      type: 'Feature',
      ...
      properties: {
        ...
        agency: g.agency,
        stage: g.stage,
        value: g.measurement?.value ?? 0,
        trend: g.measurement?.trend ?? 0,
        stateMnwMhw: g.measurement?.stateMnwMhw ?? '',
        timestamp: g.measurement?.timestamp ?? '',
      }
    })) : []);

    setGeo('train-stations', activeLayers.train_stations && data.train_stations ? data.train_stations.map((s: any) => ({
      type: 'Feature',
  ...
  useEffect(() => {
    if (!mapReady) return;
    const frequency = activeLayers.grid_frequency && data.grid_frequency?.[0];
    setGeo('grid-frequency', frequency ? [{
      type: 'Feature', geometry: { type: 'Point', coordinates: [frequency.lng, frequency.lat] }, properties: frequency,
    }] : []);
    setGeo('grid-flows', activeLayers.power_flows && data.power_flows ? data.power_flows.map((flow: any) => ({
      type: 'Feature', geometry: { type: 'LineString', coordinates: [[flow.from_lng, flow.from_lat], [flow.to_lng, flow.to_lat]] }, properties: flow,
    })) : []);
    // ENTSO-E reports units by bidding zone rather than publishing sensitive
    // plant coordinates. Spread colocated reports slightly around the zone
    // anchor so every outage remains selectable, without implying a site fix.
    setGeo('grid-outages', activeLayers.generation_outages && data.generation_outages ? data.generation_outages.map((outage: any, index: number) => {
      const angle = index * 2.399963;
      const radius = 0.18 + (index % 4) * 0.06;
      return { type: 'Feature', geometry: { type: 'Point', coordinates: [outage.lng + Math.cos(angle) * radius, outage.lat + Math.sin(angle) * radius] }, properties: outage };
    }) : []);
  }, [mapReady, data.grid_frequency, data.power_flows, data.generation_outages, activeLayers.grid_frequency, activeLayers.power_flows, activeLayers.generation_outages, setGeo]);

  useEffect(() => {
    if (!mapReady) return;
    setGeo('gas-pipelines', activeLayers.gas_pipelines && data.gas_pipelines ? data.gas_pipelines : []);
    setGeo('lng-terminals', activeLayers.lng_terminals && data.lng_terminals ? data.lng_terminals : []);
  }, [mapReady, data.gas_pipelines, data.lng_terminals, activeLayers.gas_pipelines, activeLayers.lng_terminals, setGeo]);

  useEffect(() => {
    if (!mapReady) return;
    setGeo('gnss-interference', activeLayers.gnss_interference && data.gnss_interference ? data.gnss_interference.map((zone: any) => ({
      type: 'Feature', geometry: { type: 'Point', coordinates: [zone.lng, zone.lat] }, properties: zone,
    })) : []);
    setGeo('sdr-receivers', activeLayers.sdr_receivers && data.sdr_receivers ? data.sdr_receivers.map((receiver: any) => ({
      type: 'Feature', geometry: { type: 'Point', coordinates: [receiver.lng, receiver.lat] }, properties: receiver,
    })) : []);
    setGeo('notam-alerts', activeLayers.notam_alerts && data.notam_alerts ? data.notam_alerts.map((notam: any, index: number) => {
      // FIR-scoped notices share an approximate anchor. Fan those out slightly
      // so each remains clickable without implying a precise incident point.
      const approximate = notam.position_basis !== 'notam-coordinate';
      const angle = index * 2.399963;
      const radius = approximate ? 0.12 + (index % 5) * 0.035 : 0;
      return {
        type: 'Feature',
        geometry: { type: 'Point', coordinates: [notam.lng + Math.cos(angle) * radius, notam.lat + Math.sin(angle) * radius] },
        properties: notam,
      };
    }) : []);
  }, [mapReady, data.gnss_interference, data.sdr_receivers, data.notam_alerts, activeLayers.gnss_interference, activeLayers.sdr_receivers, activeLayers.notam_alerts, setGeo]);
  ...
  useEffect(() => {
    ...
    setVis(['grid-frequency-halo','grid-frequency-dot','grid-frequency-label'], activeLayers.grid_frequency);
    setVis(['grid-flow-lines','grid-flow-label'], activeLayers.power_flows);
    setVis(['grid-outage-dots'], activeLayers.generation_outages);
    setVis(['gas-pipeline-lines'], activeLayers.gas_pipelines);
    setVis(['lng-terminal-halo','lng-terminal-dots','lng-terminal-label'], activeLayers.lng_terminals);
    setVis(['gnss-integrity-halo','gnss-integrity-dots','gnss-integrity-label'], activeLayers.gnss_interference);
    setVis(['sdr-receiver-halo','sdr-receiver-dots','sdr-receiver-label'], activeLayers.sdr_receivers);
    setVis(['notam-alert-halo','notam-alert-dots','notam-alert-label'], activeLayers.notam_alerts);
