...
const WorldRemote = dynamic(() => import('@/components/WorldRemote'), { ssr: false });
...
export default function Dashboard() {
  ...
  const [mapBootReady, setMapBootReady] = useState(false);
  const [dataBootReady, setDataBootReady] = useState(false);
  const [layerBootReady, setLayerBootReady] = useState(false);
  const splashStartedAt = useRef(Date.now());
  const handleMapReady = useCallback(() => setMapBootReady(true), []);
  ...
  useEffect(() => {
    ...
    const eqTimer = setTimeout(() => { void fetchEndpoint(eqUrl, eqTransform); }, 0);
    const newsTimer = setTimeout(() => { void fetchEndpoint('/api/news'); }, 600);
    ...
    const marketTimer = setTimeout(() => loadMarkets(), 1200);
  ...
  }, [fetchEndpoint, dataBootReady]);

  // ── LAYER-AWARE DATA LOADING — only fetch when layer is toggled ON ──
  const layerFetchedRef = useRef<Set<string>>(new Set());
  useEffect(() => {
    if (!layerBootReady || !activeLayers.cctv) return;
  ...
  }, [activeLayers.cctv, layerBootReady]);

  useEffect(() => {
    if (!layerBootReady) return;
    ...
    // Maritime
    if ((activeLayers.maritime || (activeLayers as any).maritime_routes) && !layerFetchedRef.current.has('maritime')) {
      fetchEndpoint('/api/maritime', d => ({ maritime_ports: d.ports, maritime_chokepoints: d.chokepoints, maritime_ships: d.ships, maritime_routes: d.routes }));
      layerFetchedRef.current.add('maritime');
    }
    // Rail & Transit
    if (((activeLayers as any).train_stations || (activeLayers as any).rail_corridors) && !layerFetchedRef.current.has('trains')) {
      fetchEndpoint('/api/trains/stations', d => ({ train_stations: d.stations, rail_corridors: d.corridors }));
        ...
        {showSplash && (
          ...
          >
            ...
            <div className="flex items-center justify-center gap-0 mb-3 z-[2] max-w-[92vw] whitespace-nowrap">
              {'LIVEWORLDMAP'.split('').map((letter, i) => (
                <motion.span
                  key={i}
            ...
            <div className="w-64 md:w-80 z-[2]">
              ...
              <div className="mt-3 h-4 flex items-center justify-center">
                {[
                  { text: 'LOADING BASE MAP...', delay: 0.5 },
                  { text: 'INITIALIZING CORE FEEDS...', delay: 1.8 },
                  { text: 'SYNCING ACTIVE LAYERS...', delay: 3.1 },
                  { text: 'SYSTEM READY', delay: 4.5 },
                ].map((stage, i) => (
                  <motion.span
                    key={i}
                    initial={{ opacity: 0 }}
      ...
      <motion.div initial={{ opacity: 0, y: -20 }} animate={{ opacity: 1, y: 0 }} transition={{ duration: 1, delay: 2.5 }} className={`absolute top-4 z-[200] pointer-events-none flex flex-col`} style={{ left: isMobile ? '24px' : '64px', right: '24px' }}>
        <div className="flex items-center gap-3 w-fit">
          <div className="flex flex-col items-start gap-0.5">
            <h1 className="text-lg md:text-xl font-bold tracking-[0.3em] text-[#D4AF37] font-mono">LIVEWORLDMAP</h1>
        ...
        <div className="flex items-center gap-3 mt-1.5 pl-0 min-w-0 pr-4">
