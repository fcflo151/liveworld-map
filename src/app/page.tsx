...
import SearchBar, { type LoadedSearchItem, type SearchCategory } from '@/components/SearchBar';
...
const LiveWorldMap = dynamic(() => import('@/components/LiveWorldMap'), { ssr: false });
...
function loadedMapItems(data: Record<string, unknown[]>): LoadedSearchItem[] {
  ...
  const categoryForLayer = (layer: string): SearchCategory => {
    const name = layer.toLowerCase();
    if (name.includes('station') || name.includes('train')) return 'stations';
    if (name.includes('flight') || name.includes('aircraft') || name.includes('airport')) return 'flights';
    if (name.includes('ship') || name.includes('maritime') || name.includes('vessel')) return 'ships';
    if (name.includes('news')) return 'news';
    if (name.includes('alert') || name.includes('warning') || name.includes('earthquake') || name.includes('fire') || name.includes('weather') || name.includes('incident') || name.includes('threat')) return 'alerts';
    if (name.includes('aoi')) return 'aois';
  ...
  for (const [layer, records] of Object.entries(data)) {
    ...
    for (const [index, record] of records.entries()) {
      ...
      items.push({
        ...
        category: categoryForLayer(layer),
        lat,
        lng,
...
export default function Dashboard() {
  ...
  const globalSearchItems = useMemo<LoadedSearchItem[]>(() => [
    ...loadedSearchItems,
    ...drawnPolygons.map(shape => {
      const coordinates = shape.geojson.geometry.type === 'Polygon'
        ? shape.geojson.geometry.coordinates[0]
        : shape.geojson.geometry.coordinates;
      const points = coordinates.filter(([lng, lat]) => Number.isFinite(lat) && Number.isFinite(lng));
      const fallback = points.length
        ? points.reduce((sum, [lng, lat]) => ({ lng: sum.lng + lng / points.length, lat: sum.lat + lat / points.length }), { lat: 0, lng: 0 })
        : { lat: 0, lng: 0 };
      const [lng, lat] = shape.meta?.center ?? [fallback.lng, fallback.lat];
      return {
        id: `aoi-${shape.id}`,
        label: shape.name,
        detail: `${shape.kind.toUpperCase()} · ${shape.areaKm2.toFixed(1)} km²`,
        type: 'area of interest',
        category: 'aois' as const,
        lat,
        lng,
        zoomLevel: 10,
      };
    }),
  ], [drawnPolygons, loadedSearchItems]);
  ...
  const [selectedCivilAlert, setSelectedCivilAlert] = useState<CivilAlert | null>(null);
  const [selectedWaterwayGauge, setSelectedWaterwayGauge] = useState<WaterwayGauge | null>(null);
  ...
  useEffect(() => {
    ...
    fetch('/api/grid-intel?probe=1')
      .then(r => (r.ok ? r.json() : null))
      .then(p => { if (p) setCapabilities(c => ({ ...c, entsoe: !!p.entsoe })); })
      .catch(() => { /* ENTSO-E layers remain hidden; frequency stays keyless */ });

    fetch('/api/notams?probe=1')
      .then(r => (r.ok ? r.json() : null))
      .then(p => { if (p) setCapabilities(c => ({ ...c, icao_notam: !!p.configured })); })
      .catch(() => { /* credential-gated NOTAM layer remains hidden */ });
  ...
  useEffect(() => {
    const handler = (e: KeyboardEvent) => {
      ...
      if (e.key === 'm') { setShowMarkets(p => !p); setShowWorkspace(false); setShowIntel(false); setShowAlerts(false); setShowSpaceCam(false); setShowDrawing(false); setShowDirections(false); setShowArcGIS(false); setShowRemote(false); }
      if (e.key === 'c') setShowScmPanel(p => !p);
      if (e.key === 'i') { setShowIntel(p => !p); setShowWorkspace(false); setShowMarkets(false); setShowAlerts(false); setShowSpaceCam(false); setShowDrawing(false); setShowDirections(false); setShowArcGIS(false); setShowRemote(false); }
      ...
      if ((e.ctrlKey || e.metaKey) && (e.key === 'f' || e.key === 'k')) {
        e.preventDefault();
        setShowWorkspace(false); setShowIntel(false); setShowMarkets(false); setShowAlerts(false); setShowSpaceCam(false); setShowDrawing(false); setShowDirections(false); setShowArcGIS(false); setShowRemote(false);
  ...
  useEffect(() => {
    ...
    // Flights
    if (activeLayers.flights || activeLayers.military || activeLayers.jets || activeLayers.private || activeLayers.gnss_interference) {
    ...
    if (anySatLayer) {
      ...
      loadLayerOnce('satellites', '/api/satellites', d => ({ ...d, satellites_at: d.timestamp }));
    }
    // Fires
    ...
    // Maritime
    if (activeLayers.maritime || (activeLayers as any).maritime_routes) {
      loadLayerOnce('maritime', '/api/maritime', d => ({ maritime_ports: d.ports, maritime_chokepoints: d.chokepoints, maritime_ships: d.ships, maritime_routes: d.routes }));
    }
    // Rail & Transit
    if ((activeLayers as any).train_stations || (activeLayers as any).rail_corridors) {
      loadLayerOnce('trains', '/api/trains/stations', d => ({ train_stations: d.stations, rail_corridors: d.corridors }));
    }
    // Balloons
    if (activeLayers.balloons) {
      loadLayerOnce('balloons', '/api/balloons', d => ({ balloons: d.balloons }));
    }
    // Radiation
    if (activeLayers.radiation) {
      loadLayerOnce('radiation', '/api/radiation', d => ({ radiation: d.stations }));
    }
    // Live News
    if (activeLayers.live_news) {
      loadLayerOnce('live_news', '/api/live-news', d => ({ live_feeds: d.feeds }));
    }
    // Weather
    if (activeLayers.weather) {
      loadLayerOnce('weather', '/api/weather', d => ({ weather_events: d.events }));
    }
    // Infrastructure
    if (activeLayers.infrastructure && (!layerFetchedRef.current.has('infrastructure') || !dataRef.current.infrastructure)) {
      loadLayerOnce('infrastructure', '/api/infrastructure', d => ({ infrastructure: d.infrastructure }));
    }
    // Global Incidents (GDELT)
    if (activeLayers.global_incidents) {
      loadLayerOnce('gdelt', '/api/gdelt', d => ({ gdelt: d.events }));
    }
      ...
      (async () => {
        try {
          ...
          if (res.ok) {
            ...
            layerFetchedRef.current.delete('cables');
          }
        } catch (e) {
          layerFetchedRef.current.delete('cables');
    ...
    if ((activeLayers as any).cyber_attacks) {
      loadLayerOnce('cyber_attacks', '/api/cyber-attacks', d => ({ cyber_attacks: d.attacks }));
    }
    ...
    // European grid telemetry — frequency is keyless; ENTSO-E adds flows/outages.
    if (activeLayers.grid_frequency || activeLayers.power_flows || activeLayers.generation_outages) {
      loadLayerOnce('grid_intel', '/api/grid-intel', d => ({
        grid_frequency: d.frequency ? [d.frequency] : [],
        power_flows: d.power_flows ?? [],
        generation_outages: d.generation_outages ?? [],
      }));
    }

    // Global Energy Monitor gas routes and LNG facilities (European scope).
    if (activeLayers.gas_pipelines || activeLayers.lng_terminals) {
      loadLayerOnce('energy_infrastructure', '/api/energy-infrastructure?scope=europe', d => ({
        gas_pipelines: d.pipelines?.features ?? [],
        lng_terminals: d.lng_terminals?.features ?? [],
      }));
    }

    if (activeLayers.sdr_receivers) {
      loadLayerOnce('sdr_receivers', '/api/sdr-receivers', d => ({ sdr_receivers: d.receivers ?? [] }));
    }

    if (activeLayers.notam_alerts) {
      loadLayerOnce('notam_alerts', '/api/notams', d => ({ notam_alerts: d.notams ?? [] }));
    }
  ...
  useEffect(() => {
    ...
    if (activeLayers.maritime || (activeLayers as any).maritime_routes) {
      intervals.push(setInterval(() => fetchEndpoint('/api/maritime', d => ({ maritime_ports: d.ports, maritime_chokepoints: d.chokepoints, maritime_ships: d.ships, maritime_routes: d.routes })), 10000)); // 10s
    }
    if ((activeLayers as any).cyber_attacks) {
    ...
    if (activeLayers.grid_frequency || activeLayers.power_flows || activeLayers.generation_outages) {
      intervals.push(setInterval(() => fetchEndpoint('/api/grid-intel', d => ({
        grid_frequency: d.frequency ? [d.frequency] : [],
        power_flows: d.power_flows ?? [],
        generation_outages: d.generation_outages ?? [],
      })), 60_000));
    ...
    if (activeLayers.sdr_receivers) {
      intervals.push(setInterval(() => fetchEndpoint('/api/sdr-receivers', d => ({ sdr_receivers: d.receivers ?? [] })), 900_000));
    }
    if (activeLayers.notam_alerts) {
    ...
    <main className="fixed inset-0 w-full h-full bg-[var(--bg-void)] overflow-hidden">
      ...
      <ErrorBoundary name="Map">
        <LiveWorldMap 
          ...
          onSelectCivilAlert={setSelectedCivilAlert}
          onSelectWaterwayGauge={setSelectedWaterwayGauge}
        />
      </ErrorBoundary>
      ...
      {selectedStation && (
        <TrainStationPanel
          ...
          onFocusCoordinates={(lat, lng) => setFlyToLocation({ lat, lng, zoom: 13, ts: Date.now() })}
        />
      )}

      {/* ── CIVIL PROTECTION ALERT MODAL ── */}
      {selectedCivilAlert && (
        <CivilProtectionModal
          alert={selectedCivilAlert}
          onClose={() => setSelectedCivilAlert(null)}
          onFocusCoordinates={(lat, lng) => setFlyToLocation({ lat, lng, zoom: 12, ts: Date.now() })}
        />
      )}

      {/* ── WATERWAY GAUGE PANEL ── */}
      {selectedWaterwayGauge && (
        <WaterwayGaugePanel
          gauge={selectedWaterwayGauge}
          onClose={() => setSelectedWaterwayGauge(null)}
          onFocusCoordinates={(lat, lng) => setFlyToLocation({ lat, lng, zoom: 13, ts: Date.now() })}
        />
      )}
      ...
      {/* ── HEADER ── */}
      <motion.div initial={{ opacity: 0, y: -20 }} animate={{ opacity: 1, y: 0 }} transition={{ duration: 1, delay: 2.5 }} className={`absolute top-4 z-[200] pointer-events-none flex flex-col`} style={{ left: isMobile ? '20px' : '48px', right: '24px' }}>
        <div className="flex items-center w-fit">
          {/* The retired Horus mark deliberately no longer renders in the map header. */}
          {false && (<svg viewBox="0 0 650 500" className="w-8 h-8 md:w-10 md:h-10 shrink-0 transition-colors duration-500 text-[#D4AF37] drop-shadow-[0_0_8px_rgba(255,215,0,0.5)]" fill="currentColor">
          ...
          <div className="flex flex-col items-start gap-0.5">
            <h1 className="text-lg md:text-xl font-bold tracking-[0.3em] text-[#D4AF37] font-mono">LIVEWORLD MAP</h1>
            <span className="text-[9px] md:text-[10px] font-mono tracking-[0.2em] opacity-80 uppercase text-[#D4AF37]">GLOBAL INTELLIGENCE & CIVIL PROTECTION GRID</span>
          </div>
        </div>
        <div className="flex items-center gap-3 mt-1.5 min-w-0 pr-4">
          <span className="text-[9px] md:text-[9px] text-[var(--text-muted)] font-mono tracking-[0.2em] md:tracking-[0.3em] uppercase opacity-40 truncate">
            REAL-TIME GLOBAL MONITORING <span className="hidden md:inline">· FLIGHTS · RAIL · WATERWAYS · CIVIL DEFENSE · ENERGY GRID · MARITIME · CCTV</span>
          </span>
        </div>
      </motion.div>
      ...
      <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} transition={{ delay: 3 }} className="status-bar-desktop absolute top-4 right-6 z-[200] pointer-events-none flex items-center gap-3 text-[10px] font-mono tracking-widest text-[var(--text-muted)]">
        ...
        {!isMobile && <div className="pointer-events-auto w-72 xl:w-96">
          <SearchBar
            alwaysExpanded
            variant="liquid"
            loadedItems={globalSearchItems}
      ...
      {!isMobile && <div className="absolute right-2 top-1/2 -translate-y-1/2 flex flex-col gap-2 z-[250] pointer-events-auto bg-black/40 backdrop-blur-sm p-1 rounded-full border border-white/5">
        ...
        <div className="relative group">
          <button onClick={() => setSearchFocusToken(token => token + 1)} className="relative w-8 h-8 rounded-full flex items-center justify-center transition-colors focus:outline-none focus-visible:ring-1 focus-visible:ring-white/50 hover:bg-white/10" title="Global Search — places and live entities" aria-label="Focus global search">
                ...
                <div className="px-3 pb-3">
                  ...
                  {mobilePanel === 'search' && (
                    <div className="space-y-2">
                      <SearchBar alwaysExpanded loadedItems={globalSearchItems} focusToken={searchFocusToken} onLocate={(lat, lng, zoom) => { setFlyToLocation({ lat, lng, zoom, ts: Date.now() }); setMobilePanel(null); }} />
